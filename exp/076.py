...
def calc_cv(model_paths):
    df = pd.read_csv("input/train_folds_no_dup_5_sep_hard_local_test.csv")
    ...
    for fold, model_path in enumerate(model_paths):
        ...
        dataset = Pet2Dataset(X=test_df[CFG.ID_COL].values, y=test_df[CFG.TARGET_COL].values, Meta_features=test_df[CFG.FEATURE_COLS].values)
        data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=CFG.valid_bs, num_workers=0, pin_memory=True, shuffle=False
        )        
        ...
        for b_idx, data in tqdm(enumerate(data_loader)):
            with torch.no_grad():
                ...
                output = output.detach().cpu().numpy().tolist()
                final_output.extend(output)
        y_local_test_pred.append(np.array(final_output))
        y_local_test_true.append(test_df[CFG.TARGET_COL].values)
        local_test_idx.append(test_df[CFG.ID_COL].values)
        torch.cuda.empty_cache()

    y_pred = np.concatenate(y_pred)
    y_true = np.concatenate(y_true)
    idx = np.concatenate(idx)
    overall_cv_score_ex_local_test = calc_loss(y_true, y_pred)
    logger.info(f'ex local test cv score {overall_cv_score_ex_local_test}')

    y_local_test_pred = np.concatenate(y_local_test_pred)
    y_local_test_true = np.concatenate(y_local_test_true)
    local_test_idx = np.concatenate(local_test_idx)
    local_test_cv_score = calc_loss(y_local_test_true, y_local_test_pred)
    logger.info(f'local hard test cv score {local_test_cv_score}')

    oof_df = pd.DataFrame()
    oof_df[CFG.ID_COL] = np.concatenate([idx, local_test_idx])
    oof_df['oof'] = np.concatenate([y_pred, y_local_test_pred])
    oof_df[CFG.TARGET_COL] = np.concatenate([y_true, y_local_test_true])
    cv_score = calc_loss(oof_df[CFG.TARGET_COL], oof_df['oof'])
...
# data
train = pd.read_csv("input/train_folds_no_dup_5_sep_hard_local_test.csv")
...
for fold in range(5):
    ...
    model.load_state_dict(torch.load(f'output/pretrain_006/fold-{fold}.bin'))
    print('1st stage model weight loaded.')
