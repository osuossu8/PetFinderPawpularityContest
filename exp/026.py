...
class Pet2Dataset:
    ...
    def __getitem__(self, item):
        if self.y is not None:
            ...
            targets = self.y[item]
            aux = self.aux_y[item]
            aux_targets = np.zeros(11, dtype=float)
            aux_targets[aux] = 1.0
...
class Pet2Model(nn.Module):
    ...
    def forward(self, features, metas):
        ...
        return output.squeeze(-1), torch.sigmoid(aux_out)
...
def train_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        outputs, aux_outs = model(inputs, metas)
        loss = loss_fn(outputs, targets / 100.0) * 0.5 + loss_fn(aux_outs, aux_targets) * 0.5
        if CFG.APEX:
...
def train_mixup_cutmix_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        if np.random.rand()<0.5:
            ...
            outputs, aux_outs = model(inputs, metas)
            loss = mixup_criterion([outputs, aux_outs], [new_targets[0] / 100.0, new_targets[1] / 100.0, new_targets[2], new_targets[3], new_targets[4]]) / 2
        else:
            inputs, new_targets = cutmix(inputs, targets, aux_targets, 0.4)
            outputs, aux_outs = model(inputs, metas)
            loss = cutmix_criterion([outputs, aux_outs], [new_targets[0] / 100.0, new_targets[1] / 100.0, new_targets[2], new_targets[3], new_targets[4]]) / 2
...
def calc_cv(model_paths):
    ...
    df.loc[:,'aux_target'] = df[CFG.TARGET_COL].map(lambda x: round(x, -1)//10)
...
train.loc[:,'aux_target'] = train[CFG.TARGET_COL].map(lambda x: round(x, -1)//10)

print(train['aux_target'].value_counts())
print(train.shape)
train.head()
