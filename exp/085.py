...
cat_vs_dog = pd.read_csv("input/cat_vs_dog_train_test_image_with_pseudo_label.csv")
cat_vs_dog = create_folds(cat_vs_dog, num_splits=5)
cat_vs_dog['PATH'] = 'input/cat_vs_dog/' + cat_vs_dog['image_name'].values
...
for fold in range(5):
    ...
    cat_vs_dog[CFG.TARGET_COL] = cat_vs_dog[f'pseudo_label_fold_{fold}'].values
    cat_vs_dog_df = cat_vs_dog[cat_vs_dog.kfold == fold].reset_index(drop=True)

    trn_df = train[train.kfold != fold].reset_index(drop=True)
    val_df = train[train.kfold == fold].reset_index(drop=True)

    trn_df = pd.concat([trn_df, prev_train_df, cat_vs_dog_df], 0).reset_index(drop=True)
