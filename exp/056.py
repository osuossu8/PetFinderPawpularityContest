...
class Pet2Model(nn.Module):
    def __init__(self, model_name):
        ...
        self.adoption_dense = nn.Linear(128, CFG.TARGET_DIM)

    def forward(self, features):
        x = self.model(features)
        output = self.dense(x)
        adop_out = self.adoption_dense(x)
        return output.squeeze(-1), adop_out.squeeze(-1)


class Pet1Model(nn.Module):
    def __init__(self, model_name):
        super(Pet1Model, self).__init__()    
...
def train_fn_calc_cv_interval(epoch, model, train_data_loader, valid_data_loader, device, optimizer, scheduler, best_score):
    ...
    for batch_idx, data in enumerate(tk0):
        ...
        outputs, adop_out = model(inputs)
        loss = loss_fn(outputs, targets / 100.0) * 0.5 + aux_loss_fn(adop_out, adop) * 0.5
        loss.backward()
        optimizer.step()
        scheduler.step()
        losses.update(loss.item(), inputs.size(0))
