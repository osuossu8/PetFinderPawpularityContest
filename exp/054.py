...
CFG.get_transforms = {
        'train' : A.Compose([
            A.OneOf([
                A.RandomResizedCrop(CFG.IMG_SIZE, CFG.IMG_SIZE, p=0.25, scale=(0.85, 0.95)),
                A.Resize(CFG.IMG_SIZE, CFG.IMG_SIZE, p=0.25),
                A.Compose([
...
class Pet2Model(nn.Module):
    def __init__(self, model_name):
        ...
        self.cat_dense = nn.Linear(128, CFG.TARGET_DIM)
...
def calc_cv(model_paths):
    ...
    df = pd.merge(df, cat_or_dog, on='Id')
...
cat_or_dog = pd.read_csv("input/cat_class.csv")

print(cat_or_dog['is_cat'].value_counts())

train = pd.merge(train, cat_or_dog, on='Id')
