...
def train_mixup_fn_calc_cv_interval(epoch, model, train_data_loader, valid_data_loader, device, optimizer, scheduler, best_score):
    model.train()
    losses = AverageMeter()
    scores = MetricMeter()
    tk0 = tqdm(train_data_loader, total=len(train_data_loader))
...
for fold in range(5):
    ...
    for epoch in range(CFG.epochs):
        ...
        train_avg, train_loss, valid_avg, valid_loss, best_score = train_mixup_fn_calc_cv_interval(epoch, model, train_dataloader, valid_dataloader, device, optimizer, scheduler, best_score)
