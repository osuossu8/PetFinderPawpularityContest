...
def train_mixup_cutmix_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        if np.random.rand()<0.5:
            ...
            outputs = model(inputs, metas)
            loss = cutmix_criterion(outputs, [new_targets[0] / 100.0, new_targets[1] / 100.0, new_targets[2]])
...
for fold in range(5):
    ...
    for epoch in range(CFG.epochs):
        ...
        # else:
        train_avg, train_loss = train_fn(model, train_dataloader, CFG.DEVICE, optimizer, scheduler)

        valid_avg, valid_loss = valid_fn(model, valid_dataloader, CFG.DEVICE)
        scheduler.step()
