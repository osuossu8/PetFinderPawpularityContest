...
def train_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        loss = loss_fn(outputs, targets / 100.0) * 0.5 + loss_fn(meta_outs, metas) * 0.5
        if CFG.APEX:
...
def train_mixup_cutmix_fn(model, data_loader, device, optimizer, scheduler):
    ...
    for data in tk0:
        ...
        if np.random.rand()<0.5:
            ...
            outputs, meta_outs = model(inputs, metas)
            loss = mixup_criterion(outputs, [new_targets[0] / 100.0, new_targets[1] / 100.0, new_targets[2]]) * 0.5 + loss_fn(meta_outs, metas) * 0.5
        else:
            inputs, new_targets = cutmix(inputs, targets, 0.4)
            outputs, meta_outs = model(inputs, metas)
            loss = cutmix_criterion(outputs, [new_targets[0] / 100.0, new_targets[1] / 100.0, new_targets[2]]) * 0.5 + loss_fn(meta_outs, metas) * 0.5
