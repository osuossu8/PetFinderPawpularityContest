...
class Pet2Model(nn.Module):
    def __init__(self, model_name):
        ...
        self.dense = nn.Linear(128+8, CFG.TARGET_DIM)
...
def calc_cv(model_paths):
    ...
    df['image_path'] = df[CFG.ID_COL].map(lambda x: 'input/train/' + x + '.jpg')

    df['image_size'] = df['image_path'].apply(getSize)
    df['temp_size'] = df['image_path'].apply(getDimensions)
...
train['image_path'] = train[CFG.ID_COL].map(lambda x: 'input/train/' + x + '.jpg')

train['image_size'] = train['image_path'].apply(getSize)
train['temp_size'] = train['image_path'].apply(getDimensions)
train['width'] = train['temp_size'].apply(lambda x : x[0])
train['height'] = train['temp_size'].apply(lambda x : x[1])
# train['dullness'] = train['image_path'].apply(lambda x : perform_color_analysis(x, 'black'))
# train['whiteness'] = train['image_path'].apply(lambda x : perform_color_analysis(x, 'white'))

print(train[['width', 'height', 'image_size']].head())
print(train.shape)
train.head()
